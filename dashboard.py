...
- Fast & beautiful UI
    ...
    start_date = st.date_input("Start date", value=df['Timestamp'].min().date(), min_value=df['Timestamp'].min().date(), max_value=df['Timestamp'].max().date(), key="start_date")
    end_date = st.date_input("End date", value=df['Timestamp'].max().date(), min_value=df['Timestamp'].min().date(), max_value=df['Timestamp'].max().date(), key="end_date")
...
if btn_select:
    df_filtered = df[(df['city'] == selected_city) & (df['Timestamp'].dt.date >= start_date) & (df['Timestamp'].dt.date <= end_date)]
else:
    df_filtered = df[(df['Timestamp'].dt.date >= start_date) & (df['Timestamp'].dt.date <= end_date)]

st.write(f"Records selected: {len(df_filtered)}")

# Stylish Section Buttons
...
def send_sms_alert(body, to_number):
    try:
        ...
        from_number = st.secrets.get("twilio_from_number", "")
        if not (account_sid and auth_token and from_number and to_number):
            return False, "Missing Twilio credentials or phone number."
        client = Client(account_sid, auth_token)
        message = client.messages.create(body=body, from_=from_number, to=to_number)
        return True, f"SMS alert sent (SID: {message.sid})"
...
if section == "Pollution Trends":
    ...
    for pol in ['PM2.5', 'PM10', 'NO2', 'SO2', 'CO', 'Ozone']:
        if pol in selected_city_data.columns:
            max_val = selected_city_data[pol].max()
            thr = thresholds[pol]
            if pd.notnull(max_val) and max_val > thr:
                alerts.append(f"{pol}: Max {max_val:.1f} (> threshold {thr})")
    if not selected_city_data.empty:
        ...
        heat_gradient = {0.0: 'blue', 0.2: 'lime', 0.5: 'yellow', 0.7: 'orange', 1.0: 'red'}
        heat_data = [
        ...
        if heat_data:
            ...
            HeatMap(heat_data, radius=12, blur=16, gradient=heat_gradient, max_val=max_val).add_to(m)
        st_folium(m, width=800, height=500)
        if alerts:
            alert_msg = ";  ".join(alerts)
            st.warning(f"Alerts: {alert_msg}")
            if enable_sms and phone_number.strip():
                sms_body = f"EnviroScan Alert for {selected_city}:\n" + "\n".join(alerts)
                success, msg = send_sms_alert(sms_body, phone_number)
                if success:
                    st.success(msg)
                else:
                    st.error("Failed to send SMS alert: " + msg)
        else:
